...
//  }
  ...
  protected[loadBalancer] def processCompletion(response: Either[ActivationId, WhiskActivation],
                                                tid: TransactionId,
                                                forced: Boolean,
...
//        } else {
        schedulingState.invokerSlots
          .lift(invoker.toInt)
          .foreach(_.release(entry.fullyQualifiedEntityName, entry.maxConcurrent, entry.memory.toMB.toInt))
//        }
...
object ShardingContainerPoolBalancer extends LoadBalancerProvider {
  ...
  def schedule(maxConcurrent: Int,
    ...
    if (numInvokers > 0) {
      ...
      if (invoker.status.isUsable && dispatched(invoker.id.toInt).tryAcquire(fqn, maxConcurrent, slots, 0)) {
        Some(invoker.id)
      } else {
        ...
        if (stepsDone == numInvokers + 1) {
          ...
          if (healthyInvokers.nonEmpty) {
            ...
            dispatched(random.toInt).forceAcquire(fqn, maxConcurrent, slots)
...
case class ShardingContainerPoolBalancerState(
    ...
    if (oldSize != newSize) {
      ...
      if (oldSize < newSize) {
        ...
        _invokerSlots = _invokerSlots ++ IndexedSeq.fill(newSize - oldSize) {
          new NestedSemaphore[FullyQualifiedEntityName](currentInvokerThreshold.toMB.toInt)
        }
      }
    }
  ...
  def updateCluster(newSize: Int): Unit = {
    ...
    if (_clusterSize != actualSize) {
      ...
      _invokerSlots =
        _invokerSlots.map(_ => new NestedSemaphore[FullyQualifiedEntityName](currentInvokerThreshold.toMB.toInt))
